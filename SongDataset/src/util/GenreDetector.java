...
public class GenreDetector {
	 ...
	 */
	public HashMap<Integer, List<Float>> detectGenre(String[] terms, float[] importances)
	{
		float maxImportance = 0.0f;
		Genre maxGenre = Genre.OTHER;
		float[] genreImportances = new float[GENRES_COUNT];
		List<Float> genreScores = new ArrayList<Float>(Genre.values().length);
		...
		for(int i = 0; i < GENRES_COUNT; i++) {
			if(maxImportance != 0) {
				genreScores.add((100 / maxImportance) * genreImportances[i]);	
			} else {
				genreScores.add(0.0f);
			}
		}

		if(maxImportance == 0) {
			genreScores.add(maxGenre.ordinal(), 100.0f);
		}
		
		hashMap.put(maxGenre.ordinal(), genreScores);
